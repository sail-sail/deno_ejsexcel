...
import * as xml2json from "./lib/xml2json/index.js";
...
export async function renderExcel(exlBuf, _data_, opt) {
  ...
  for (i = m = 0, len2 = sheetEntries.length; m < len2; i = ++m) {
    ...
    if(opt && opt.cache !== false) {
      try {
        str2 = localStorage.getItem("ejsexcel_" + md5Str2);
      } catch(errTmp) {
      }
    }
    if(!str2) {
      ...
      if (sheetObj.worksheet.dataValidations) {
        if (!Array.isArray(sheetObj.worksheet.dataValidations.dataValidation)) {
      ...
      if(opt && opt.cache !== false) {
        localStorage.setItem("ejsexcel_" + md5Str2, str2);
      }
    }
    anonymous = eval(str2);
...
export function charPlus(str, num) {
...
export function num2Date(valTmp) {
  let date = undefined;
